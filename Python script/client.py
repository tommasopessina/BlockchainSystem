#This script set the dss address in the client's contract
#client
...
if len(sys.argv)!=3:	
 print("Please insert (1) client address (2) dss address")
 sys.exit(0)

address = Web3.toChecksumAddress(str(sys.argv[1]))
dssAddress = Web3.toChecksumAddress(str(sys.argv[2]))

print("dss:",dssAddress)
...
contract_data = mContract.functions.setdssAddress(dssAddress).buildTransaction({'from': account,
                                                          'gas': 1728712,
...
'''
#  function addEnergyUsed(string memory value,uint energy, string ts, string memory topic, uint option)
print("-----TEST-ADD-DATA-------")
contract_data = mContract.functions.addEnergyUsed("0.01",1,"201910220924","home1/sim1",0).buildTransaction({'from': account,
                                                          'gas': 1728712,
                                                          'gasPrice': web3.toWei('21', 'gwei')})
contract_data["to"] = address
contract_data["nonce"] = web3.eth.getTransactionCount(account)
...
'''
'''
contract_data = mContract.functions.getAllDataId().buildTransaction({'from': account,
                                                          'gas': 1728712,
                                                          'gasPrice': web3.toWei('21', 'gwei')})
contract_data["to"] = address
contract_data["nonce"] = web3.eth.getTransactionCount(account)
